#!/usr/bin/env python3
"""
Workflow completo para Advogado:
1. Buscar processos por nome do advogado
2. Extrair dados completos e movimentações de cada processo
3. Salvar em output/advogado_completo/
"""
import asyncio
import sys
import os
...
async def workflow_advogado(lawyer_name, start_process=1, end_process=None, max_workers=3):
    """Executa workflow completo para Advogado
    ...
    print("=" * 80)
    print("🏛️  WORKFLOW COMPLETO - ADVOGADO")
    print(f"👨‍⚖️  Advogado: {lawyer_name}")
    ...
    # Diretório de saída
    output_dir = Path("output/advogado_completo")
    ...
    logger.info("🚀 Iniciando workflow Advogado")

    # PASSO 1: Buscar processos por nome do advogado
    logger.info(f"🔍 PASSO 1: Buscando processos para '{lawyer_name}'")

    async with TJSPScraper() as scraper:
        # Buscar processos
        processos_encontrados = await scraper.search_by_lawyer(lawyer_name)

        if not processos_encontrados:
            logger.error("❌ Nenhum processo encontrado para o advogado especificado")
            return False
    ...
    # Semaphore para controlar concorrência máxima
    ...
    relatorio = {
        "workflow": "Advogado - Dados Completos",
    ...
    print("=" * 80)
    print(f"👨‍⚖️  Advogado pesquisado: {lawyer_name}")
    ...
    logger.info("🎉 Workflow Advogado concluído com sucesso!")
    return True


async def main():
    """Função principal"""
    parser = argparse.ArgumentParser(description='Workflow Advogado - Extração de dados processuais')
    parser.add_argument(
        'lawyer_name',
        help='Nome do advogado para buscar processos'
    )
    parser.add_argument(
        '--start',
        type=int,
    ...
    sucesso = await workflow_advogado(args.lawyer_name, start_process=args.start, end_process=args.end, max_workers=args.workers)
