-- =============================================
-- Script: 060_Fix_SP_GetKanbanDashboard.sql
-- Descrição: Corrige a SP para considerar cards concluídos no período
-- Autor: Sistema
-- Data: 2025-11-05
-- Versão: 1.0
-- =============================================
...
PRINT '================================================================================';
PRINT 'Corrigindo Stored Procedure [reports].[sp_GetKanbanDashboard]';
PRINT '================================================================================';
...
BEGIN
    ...
    -- 4. CTE para calcular os tempos de Lead e Cycle
    CardTimings AS (
        SELECT
            CardID,
    ...
    (
        ...
        SELECT
            ...
            AVG(CASE 
                WHEN ct.CompletedDate IS NOT NULL 
                     AND CAST(ct.CompletedDate AS DATE) BETWEEN @StartDate AND @EndDate
                THEN DATEDIFF_BIG(SECOND, ct.CreatedAt, ct.CompletedDate) 
                ELSE NULL 
            ...
            AVG(CASE 
                ...
                THEN DATEDIFF_BIG(SECOND, ct.StartDate, ct.CompletedDate) 
                ELSE NULL 
                ...
                THEN (
                    SUM(CASE 
                        WHEN ct.CompletedDate IS NOT NULL 
                             ...
                             AND ct.CompletedDate <= ct.DueDate 
                        THEN 1 
                        ELSE 0 
                    END) * 1.0
                ) / COUNT(DISTINCT CASE 
...
PRINT '✅ SP [reports].[sp_GetKanbanDashboard] corrigida com sucesso!';
PRINT '';
PRINT 'Teste:';
PRINT 'EXEC [reports].[sp_GetKanbanDashboard] @StartDate=''2025-10-01'', @EndDate=''2025-11-30'', @CompanyID=1';
GO
